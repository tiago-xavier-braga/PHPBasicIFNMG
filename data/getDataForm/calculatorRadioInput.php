<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>GetData</title>
    <style>
        body {
            margin: 0;
        }

        main {
            width: 100%;
            height: 100vh;
            display: flex;
            flex-direction: column;
            align-items: center;
            justify-content: center;
        }

        form {
            display: flex;
            flex-direction: column;
            align-items: center;
            justify-content: center;
            width: 30%;
            min-height: 40vh;
            background-color: #eee;
            border-radius: 10px;
        }
        input,
        button, select {
            margin-top: 2vh;
            height: 3vh;
            border: none;
            border: 1px solid #333;
            border-radius: 5px;
            padding: 4px;
        }
        .radioBox{
            display: flex;
            flex-direction: row;
            margin: 2vh 0 2vh 0;
        }

    </style>
</head>

<body>
    <main>
        <form action="calculatorRadioInput.php">
            <input type="number" name="numberOne" id="numberOne" placeholder="Number">
            <div class="radioBox">
                <input type="radio" name="operation" id="Somar" value="+">
                <label for="Somar">Somar</label>
                <input type="radio" name="operation" id="Subtrair" value="-">
                <label for="Subtrair">Subtrair</label>
                <input type="radio" name="operation" id="Multiplicar" value="*">
                <label for="Multiplicar">Multiplicar</label>
                <input type="radio" name="operation" id="Dividir" value="/">
                <label for="Dividir">Dividir</label>
            </div>

            <input type="number" name="numberTwo" id="numberTwo" placeholder="Number">
            <input type="submit" value="Calculator">
            <?php
            if (isset($_GET['numberOne']) && isset($_GET['operation']) && isset($_GET['numberTwo'])) {
                # code...
                $numberOne = $_GET['numberOne'];
                $numberTwo = $_GET['numberTwo'];
                $operation = $_GET['operation'];

                if($operation == '+'){
                    $result = $numberOne + $numberTwo;
                    echo '<h2>' . $result . '</h2>';
                }
                if($operation == '-'){
                    $result = $numberOne - $numberTwo;
                    echo '<h2>' . $result . '</h2>';
                }
                if($operation == '*'){
                    $result = $numberOne * $numberTwo;
                    echo '<h2>' . $result . '</h2>';
                }
                if($operation == '/'){
                    $result = $numberOne / $numberTwo;
                    echo '<h2>' . $result . '</h2>';
                }
            }
            ?>
        </form>
    </main>
</body>

</html>